...
class EuroSAT(DatasetBase):
    ...
    def __init__(self, root, shot, seed, labels_list=None, model_name=None, subsample="all"):
        ...
        # Update file paths to include hierarchy information and model name
        self.split_path = os.path.join(self.dataset_dir, f"split_EuroSAT_{n_layers}layers{model_suffix}.json")
        ...
        if labels_list is not None:
            ...
            # Store the mapping of the first level
            ...
            for idx, subclasses in superclass_to_subclasses.items():
                if subclasses:  # ensure there are subclasses
            ...
            for level in range(1, n_layers):
                ...
                # Store the mapping of the current level
                self.subclass_to_superclass_tensor.append(subclass_to_current_idx)
                self.hierarchy[level] = current_superclass_names

                # Build names for current-level superclasses by concatenating all base subclass names with "or"
                current_names_dict = {}
    ...
    def read_data(self):
        ...
        for label, category in enumerate(categories):
            ...
            # Ensure each split contains data
            if n_train <= 0 or n_val <= 0 or n_test <= 0:
                print(f"Warning: Category {category} has insufficient images ({n_total}).")
                if n_total < 3:
                    # If there are too few images, put all of them into the training set
                    n_train = n_total
                    n_val = 0
                    n_test = 0
                else:
            ...
            # Use NEW_CNAMES to map category names
            class_name = NEW_CNAMES.get(category, category)

            train.extend(_collate(images[:n_train], label, class_name))
            if n_val > 0:
                val.extend(_collate(images[n_train:n_train + n_val], label, class_name))
            if n_test > 0:
                test.extend(_collate(images[n_train + n_val:], label, class_name))
