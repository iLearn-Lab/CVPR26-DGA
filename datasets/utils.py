...
class Datum:
    ...
    Args:
        ...
        label (int or list): class label(s). Can be a single label or a list of [family_label, class_label].
...
class DatasetBase:
    ...
    def get_num_classes(data_source):
        ...
        """
        # 检查第一个元素的标签类型
        if isinstance(data_source[0].label, list):
            # 对于多层级标签，返回每个层级的类别数
            num_classes = []
            for i in range(len(data_source[0].label)):
                label_set = set()
                for item in data_source:
                    label_set.add(item.label[i])
                num_classes.append(len(label_set))
            return num_classes
        else:
            # 原始处理方式
            label_set = set()
    ...
    def get_lab2cname(data_source):
        ...
        for item in data_source:
            ...
            if isinstance(label, list):
                label = tuple(label)
            container.add((label, item.classname))
        mapping = {label: classname for label, classname in container}
        labels = list(mapping.keys())
        labels.sort()  # 排序标签（支持 tuple 的自然排序）
    ...
    def split_dataset_by_label(self, data_source):
        """Split dataset by subclass label (ignore family label)."""
        output = defaultdict(list)

        for item in data_source:
            y = item.label

            # 支持 int / tuple / list 三种情况
            if isinstance(y, (list, tuple)):
                subclass = int(y[0])
            else:
                subclass = int(y)

            output[subclass].append(item)
